...
describe("utils", () => {
  ...
  test("getTailName returns last segment", () => {
    expect(getTailName("/databases/starter-basic.kdb")).toBe(
      "starter-basic.kdb",
    );
  ...
  test("buildDeckDisplayNameFromFileName strips extension and separators", () => {
    expect(buildDeckDisplayNameFromFileName("math-geometry.kdb")).toBe(
      "math geometry",
    );
    expect(buildDeckDisplayNameFromFileName("french_travel.sqlite3")).toBe(
      "french travel",
    );
  });
